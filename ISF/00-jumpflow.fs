...
float fractalWave(vec2 p, float t) {
    ...
    for (int i = 0; i < 6; i++) {
        if (i >= int(fractalDepth)) break;
        // Add directional motion toward the camera with layered chaos
    ...
    // Add global z-oscillation for surge/recede effect
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float h0 = fractalWave(waveUV + vec2(0.02, 0.0), t) * zDepthScale; // Increased offset for stronger normals
    float h1 = fractalWave(waveUV + vec2(0.0, 0.02), t) * zDepthScale;
    vec3 normal = normalize(vec3(h0 - height, h1 - height, 0.05)); // Reduced z-component for sharper normals
    normal.z *= zDepthScale * 0.5; // Amplify normal z for more pronounced 3D effect
