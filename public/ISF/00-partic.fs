...
vec2 getParticlePos(int i, float t) {
    ...
    vec2 basePos = vec2(h * 2.0 - 1.0, fract(h * 123.45) * 2.0 - 1.0);
    vec2 videoUV = (basePos + 1.0) * 0.5;
    vec3 videoSample = texture2D(videoInput, videoUV).rgb;
    float brightness = dot(videoSample, vec3(0.299, 0.587, 0.114));
    basePos += vec2(cos(brightness * TWO_PI), sin(brightness * TWO_PI)) * videoInfluence * 0.2;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Central core glow (keep some softness for the core)
    float coreDist = length(cent);
    float coreGlow = smoothstep(0.2, 0.0, coreDist);
    vec3 coreColor = texture2D(videoInput, uv).rgb; // Sample video for core color
    ...
    for (int i = 0; i < 300; i++) {
        ...
        // Draw particle with crisp vector-like rendering
        float d = length(cent - p1);
        float aa = 0.001; // Minimal anti-aliasing factor
        float particleEdge = particleSize - aa;
        float particleAlpha = 1.0 - smoothstep(particleEdge, particleSize, d); // Sharper edge with minimal AA
        vec2 particleUV = (p1 + 1.0) * 0.5;
        particleUV = clamp(particleUV, 0.0, 1.0);
        vec3 particleColor = texture2D(videoInput, particleUV).rgb;
        col += particleColor * particleAlpha * 0.8; // Slightly brighter for visibility

        // Connect to nearby particles with crisp vector-like lines
        ...
        for (int j = startJ; j < endJ; j++) {
            ...
            if (dist < connectDistance) {
                ...
                float lineEdge = lineThickness - aa;
                float lineAlpha = 1.0 - smoothstep(lineEdge, lineThickness, lineDist); // Sharper edge with minimal AA
                vec2 lineUV = mix(particleUV, (p2 + 1.0) * 0.5, h);
                lineUV = clamp(lineUV, 0.0, 1.0);
                vec3 lineColor = texture2D(videoInput, lineUV).rgb;
                col += lineColor * lineAlpha * 0.5; // Adjusted intensity for clarity
            }
        }
    }
